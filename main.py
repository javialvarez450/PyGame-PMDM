...
class Juego:
    def __init__(self):
        ...
        #Vida y puntuación
        self.vidas = 3
        self.live_surf = pygame.image.load('resources/443-4431365_spaceship-sprite-png-transparent-png.png').convert_alpha()
        self.live_x_start_pos = ancho_pantalla - (self.live_surf.get_size()[0] * 2 + 20)
        #Alien
        ...
        #laser del jugador
                if self.jugador.sprite:
                    for laser in self.jugador.sprite.lasers:
                        if pygame.sprite.spritecollide(laser,self.marcianos,True):
                            laser.kill()

                        if pygame.sprite.spritecollide(laser,self.extraNave,True):
                            laser.kill()
                #laser de los marcianos
                if self.marciano_lasers:
                    for laser in self.marciano_lasers:
                        if pygame.sprite.spritecollide(laser, self.jugador, False):
                            ...
                            if self.vidas <= 0:
                                ...
                                sys.exit()
                if self.marcianos:
                    for marciano in self.marcianos:
                        if pygame.sprite.spritecollide(marciano, self.jugador, False):
                            pygame.quit()
                            sys.exit()
    def dvidas(self):
        for vida in range (self.vidas - 1):
            x = self.live_x_start_pos + (vida * (self.live_surf.get_size()[0]+10))
            pantalla.blit(self.live_surf,(x,8))
